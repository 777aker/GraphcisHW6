/*
	...
	Copy of HW5 but now there is...texturessssss
...
// texture stuff woooo
// this is kinda getting ridiculously big
// I'll fix it up some other time hehe
unsigned int texture[9];
// some constants to make things easier
// (basically I made an enum)
const int groundtex = 0;
const int branchtex = 1;
const int fencetex = 2;
const int grassestex = 3;
const int leavestex = 4;
const int mainbranchtex = 5;
const int maintrunktex = 6;
const int trunktex = 7;
const int weirdleavestex = 8;
void loadtextures();
...
void display() {
	...
	// enable textures
	// I want modular bc I've worked so hard on cool colors
	...
	// culling on it
	// I do texture here bc hw6functions doesn't have access
	// to the texture array
	// shouldn't really make a difference that it's here
	glBindTexture(GL_TEXTURE_2D, texture[grassestex]);
	grass();
	glEnable(GL_CULL_FACE);
	...
	// the texture is the entire ground and the ground
	// is split into 4 quads so actually gotta split the texture
	// into 4 parts which is kinda annoying but oh well
	// looks cool
	glTexCoord2f(0,1); glVertex3f(-50, ground, 50);
	glTexCoord2f(.5,1); glVertex3f(0, ground, 50);
	glTexCoord2f(.5,.5); glVertex3f(0, ground, 0);
	glTexCoord2f(0,.5); glVertex3f(-50, ground, 0);

	glTexCoord2f(.5,1); glVertex3f(0, ground, 50);
	glTexCoord2f(1,1); glVertex3f(50, ground, 50);
	glTexCoord2f(1,.5); glVertex3f(50, ground, 0);
	glTexCoord2f(.5,.5); glVertex3f(0, ground, 0);

	glTexCoord2f(0,.5); glVertex3f(-50, ground, 0);
	glTexCoord2f(.5,.5); glVertex3f(0, ground, 0);
	glTexCoord2f(.5,0); glVertex3f(0, ground, -50);
	glTexCoord2f(0,0); glVertex3f(-50, ground, -50);

	glTexCoord2f(.5,.5); glVertex3f(0, ground, 0);
	glTexCoord2f(1,.5); glVertex3f(50, ground, 0);
	glTexCoord2f(1,0); glVertex3f(50, ground, -50);
	glTexCoord2f(.5,0); glVertex3f(0, ground, -50);
	...
	glBindTexture(GL_TEXTURE_2D, texture[fencetex]);
	fence();
	picket();
	...
	*/
	...
	glWindowPos2i(5, 5);
	Print("Change Intensity with < > make it .05 for non weirdness");
	glWindowPos2i(5,25);
	Print("Intensity: %f", lf);
...
int main(int argc, char* argv[]) {
	...
	glutCreateWindow("Kelley Kelley HW6");
	#ifdef USEGLEW
	//  Initialize GLEW
